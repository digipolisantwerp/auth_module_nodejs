...
module.exports = function createService(config) {
  function createOAuth(serviceName) {
    //TODO: dit ook in de config steken?
    return new OAuth2(
      config.auth.clientId,
      config.auth.clientSecret,
      config.apiHost,
      null,
      `/astad/${serviceName}/v1/oauth2/token`,
      null
    );
  }
  ...
  function loginUser(code, serviceName, callback) {
    ...
    oauth.getOAuthAccessToken(code, {
      ...
      if (config.serviceProviders.mprofiel && config.serviceProviders.mprofiel.fetchPermissions) {
