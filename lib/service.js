...
module.exports = function createService(config) {
  ...
  function createToken(results, refreshToken, serviceName, issuedDate) {
    return {
  ...
  function requestPermissionsWithToken(token, cb) {
    ...
    request({
      url: `${config.apiHost}/acpaas/meauthz/v2/permissions?applicationId=${config.applicationName}`,
      auth: {
        bearer: token
      },
      headers: {
  ...
  function loginUser(code, serviceName, callback) {
    ...
    oauth.getOAuthAccessToken(code, {
      ...
      async.parallel(actions, (error, results) => {
        ...
        return callback(null, user, createToken(OAuthToken, refreshToken, serviceName, new Date()));
      });
    });
  }
  ...
  function refresh(token, service, callback) {
    ...
    oauth.getOAuthAccessToken(token.refreshToken, {
      ...
      return callback(null, createToken(OAuthToken, refreshToken, token.serviceName, token.issuedDate));
    });
  }
