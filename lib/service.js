...
module.exports = function createService(config) {
  function createOAuth(service) {
    return new OAuth2(
      config.auth.clientId,
      config.auth.clientSecret,
      config.apiHost,
      null,
      `/astad/${service}/v1/oauth2/token`,
      null
    );
  }

  function createToken(results, refreshToken) {
    return {
  ...
  function requestUserWithToken(token, service, callback) {
    const url = `${config.apiHost}/astad/${service}/v1/v1/me`;
    request({
      url: url,
      auth: {
        bearer: token
  ...
  function loginUser(code, serviceIdentifier, callback) {
    const service = serviceIdentifier.indexOf('aprofiel') > -1 ? 'aprofiel' : 'mprofiel'
    ...
    oauth.getOAuthAccessToken(code, {
      ...
      const actions = {
        user: cb => requestUserWithToken(accessToken, service, cb)
      };
  ...
  function refresh(token, callback) {
    ...
    const service = config.auth.service.indexOf('aprofiel') > -1 ? 'aprofiel' : 'mprofiel'
    const oauth = createOAuth(service);
