...
module.exports = function createController(config) {
  ...
  function shouldRefreshToken(token, serviceProviderConfig) {
    return (!token.issuedDate || !serviceProviderConfig.maxAgeRefreshToken || new Date(token.issuedDate).getTime() + (serviceProviderConfig.maxAgeRefreshToken * 1000) > new Date().getTime())
