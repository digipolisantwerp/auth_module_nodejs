...
var cookieParser = _interopDefault(require('cookie'));
...
function getNewAccessToken(clientId, clientSecret, url) {
  const oauth2 = new oauth.OAuth2(clientId, clientSecret, url, null, ACCESS_TOKEN_PATH);
...
function loadRoutes(app, config) {
  ...
  const router = new express.Router(); // warning printen als trust proxy niet enabled is? of standaard enablen?
  ...
  router.post(`${basePath}/event/loggedout`, express.json(), loggedoutEvent);
